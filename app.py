...
if not st.session_state.password_valid:
    ...
    n_samples = st.sidebar.slider(
        ...
        max_value=100,
        value=40,
        help="Nombre de morceaux de documents provenant du site DRIAS récupérés pour chaque question.\nPlus le nombre est grand, plus le chatbot aura de contexte mais plus le temps de calcul sera long.\nLorsque le nombre de morceaux est élevé, il est recommandé d'utiliser un modèle de reranking.",
    )
    use_rag = st.sidebar.checkbox(
        "Use rag",
        value=True,
        help="Utiliser le RAG (Retrieval augmented generation) permet de générer des réponses plus précises en se basant sur des morceaux de documents récupérés.\nSi cette option est désactivée, le chatbot générera des réponses sans se baser sur le site DRIAS.",
    )
    ...
    reranker_model = st.sidebar.selectbox(
        ...
        help="Modèle de reranking utilisé selctionner les morceaux de documents les plus important parmis ceux recupérés et les classer par ordre de pertinence. \nUtiliser un modèle de reranking permet d'améliorer la qualité des réponses mais augmente le temps de calcul.\nIl est recommandé d'utiliser un modèle de reranking lorsque le nombre de morceaux de documents récupérés est élevé.",
    )
